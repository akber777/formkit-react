...
>({ children, defaultSubmitBtn = true, ...rest }: ValidateProps<T>) {
  ...
  const getFormData = (formElement: HTMLFormElement) => {
    const formInputs = Array.from(formElement.querySelectorAll("*")).filter(
      (element) => {
    ...
    formInputs.forEach((element) => {
      ...
      const name =
        input.getAttribute("name") || input.getAttribute("data-name");
    ...
    if (typeof params === "object" && params !== null) {
      ...
      Object.entries(params).forEach(([name, value]) => {
        ...
        const controllerElement = document.querySelector(
          `[data-formkit-controller][data-name="${name}"]`
        );
